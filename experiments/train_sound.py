...
class Trainer(torch.nn.Module):
    ...
    def forward(self, target, it):
        ...
        return self.geometry.tick(glctx, target, self.light, self.material, self.image_loss_fn, it, FLAGS)
...
def optimize_mesh(
    ...
    learning_rate_pos = FLAGS.learning_rate_pos
    learning_rate_mat = FLAGS.learning_rate_mat
    ...
    for it, target in enumerate(dataloader_train):
        ...
        if it % log_interval == 0 and FLAGS.local_rank == 0:
            ...
            print("iter=%5d, img_loss=%.6f, reg_loss=%.6f, audio_loss=%.6f, lr=%.5f, time=%.1f ms, rem=%s" % 
                (it, img_loss_avg, reg_loss_avg, audio_loss_avg, optimizer.param_groups[0]['lr'], iter_dur_avg*1000, util.time_to_text(remaining_time)))
...
if __name__ == "__main__":
    ...
    ref_tetmesh = meshio.read(FLAGS.ref_mesh[:-4] + ".msh")
    
    dataset_train    = DatasetMesh(ref_mesh, glctx, RADIUS, FLAGS, validate=False, fixed_obs=FLAGS.fixed_obs, ref_tetmesh=ref_tetmesh)
    dataset_validate = DatasetMesh(ref_mesh, glctx, RADIUS, FLAGS, validate=True, fixed_obs=FLAGS.fixed_obs, ref_tetmesh=ref_tetmesh)
    ...
    if FLAGS.base_mesh is None:
        ...
        if FLAGS.init_mesh:
            geometry = train_sdfnerf(geometry, FLAGS.init_mesh_dir, FLAGS)
