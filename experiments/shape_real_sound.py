...
# original 1st mesh need to rescale
mesh = TetMesh.from_triangle_mesh(mesh_dir + "model.stl")
vertices = mesh.vertices.clone()
tets = mesh.tets.clone()
gt_scale = torch.tensor([1.0, 1.0, 1.2], dtype=torch.float64).cuda()
...
model = build_model(mesh_dir=None, mode_num=eigen_num, order=mesh_order, mat=material_coeff, task=task, \
    scale_range=scale_range, init_scale=init_scale, vertices=vertices, tets=tets)
# mat_model = TrainableLinear
# # Create the pipeline object(model, oscillator)
# model = DiffSoundObj(mesh_dir, mode_num=eigen_num, order=mesh_order,
#                      mat=material_coeff, mat_model=mat_model)
...
for epoch_i in tqdm(range(max_epoch)):
    ...
    if epoch_i == early_loss_epoch: # now change material model to train poisson # shape needs it too!
