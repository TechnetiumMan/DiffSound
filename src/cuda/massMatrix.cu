                                           ...
                                           int order)
{
    ...
    double V = ((x[1] - x[0]) * ((y[2] - y[0]) * (z[3] - z[0]) - (y[3] - y[0]) * (z[2] - z[0])) +
...
float get_shape_func_value(int order, int i, float x, float y, float z, float w)
{
    ...
    else if (order == 3)
    {
        switch (i)
        {
            ...
            case 4:
                return 9 / 2 * y * z * (3 * y - 1);
            case 5:
                return 9 / 2 * y * z * (3 * z - 1);
            case 6:
                return 1 / 2 * (3 * z - 1) * (3 * z - 2) * z;
            case 7:
                return 9 / 2 * z * x * (3 * z - 1);
            case 8:
                return 9 / 2 * z * x * (3 * x - 1);
            case 9:
                return 27 * x * y * z;
            case 10:
                return 9 / 2 * x * w * (3 * x - 1);
            case 11:
                return 9 / 2 * y * w * (3 * y - 1);
            case 12:
                return 9 / 2 * z * w * (3 * z - 1);
            case 13:
                return 9 / 2 * x * w * (3 * w - 1);
            case 14:
                return 9 / 2 * y * w * (3 * w - 1);
            case 15:
                return 9 / 2 * z * w * (3 * w - 1);
            case 16:
                return 1 / 2 * (3 * w - 1) * (3 * w - 2) * w;
            case 17:
                return 27 * y * z * w;
            case 18:
                return 27 * x * z * w;
                          ...
                          const int order)
{
    ...
    GArr<float> m((float *)element_mm.data_ptr(), element_mm.size(0));

    int tet_num = tets.size() / vnum;
    cuExecute(tet_num, compute_mass_matrix_kernel, values, rows, cols, vertices, tets, density, m, order);
}
