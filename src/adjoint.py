...
class AdjointSolver():
    ...
    def get_grad(self, x, dA_dtheta):
        ...
        for i in tqdm(range(self.num_step)): # notice that because lambda(num_step)=0, don't need to calculate to num_step+1
            ...
            df_dtheta = df_dA @ dA_dtheta # (num_points, num_theta)
            add_term = self.lbd[:, i] @ df_dtheta
            dL_dtheta += add_term # integration!
            
        return dL_dtheta
    
def calculate_A_grad(M, C, K, dM, dC, dK):
    ...
    dA = torch.zeros((dM.shape[0], 2*num_points, 2*num_points)).cuda()
